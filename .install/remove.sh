...
# Remove Swaylock
if [[ $(_isInstalledParu "swaylock-effects-git") == 0 ]]; then
    paru --noconfirm -Rns swaylock-effects-git
    echo ":: swaylock removed"
    echo
fi

# Remove rofi-lbonn-wayland
if [[ $(_isInstalledParu "rofi-lbonn-wayland") == 0 ]]; then
    paru --noconfirm -Rns rofi-lbonn-wayland
    echo ":: rofi-lbonn-wayland removed"
    echo
fi

# Remove hypridle-bin
if [[ $(_isInstalledParu "hypridle-git") == 0 ]]; then
    paru --noconfirm -Rns hypridle-git
...
# Remove hyprlock-bin
if [[ $(_isInstalledParu "hyprlock-git") == 0 ]]; then
    paru --noconfirm -Rns hyprlock-git
