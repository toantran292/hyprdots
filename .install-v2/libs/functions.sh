...
_isInstalled() {
  ...
  case $pm in
  "pacman") check="$(sudo pacman -Qs --color always "${package}" | grep "local" | grep "${package} ")" ;;
...
_installPackages() {
  ...
  # [ $? != 0 ] && echo "$0: Error when getopt, please recheck parameters."
  # eval set -- "$param"
  # echo $param
  # echo $1
  # echo $force
  ...
  case $pm in
  "pacman") sudo pacman --noconfirm -S "${toInstall[@]}" ;;
  *) yay --noconfirm -S "${toInstall[@]}" ;;
  esac
}
_installPackagesPacman() {
  _installPackages "pacman" $@
}

_installPackagesYay() {
  _installPackages "yay" $@
}

_forcePackagesPacman() {
  _installPackages -f "pacman" $@
}

_forcePackagesYay() {
  _installPackages -f "yay" $@
}
